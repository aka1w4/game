...
Button::Button(int x, int y, int w, int h, int fontsize, const char *text,
               Texture2D &img, std::function<void()> action)
    : x(x), y(y), w(w), h(h), fontsize(fontsize), img(&img), text(text),
      action(action) {}
...
void Button::Draw(int offScrollY) {
  ...
  bool inPos = CheckCollisionPointRec(
      GetMousePosition(),
      Rectangle{float(screenX), float(screenY), float(w2), float(h2)});

  Rectangle src =
      inPos ? Rectangle{0, 31, float(w), float(h)}
            // jika inPos = true button akan berwarna abu2
            : Rectangle{
                  0, 0, float(w),
                  float(h)}; // jika inPos = false button akan berwarna hitam
...
void WorldButton::DrawDelete(int offScrollY) {
  ...
  DrawTexturePro(*imgicon, Rectangle{16, 48, 16, 16},
                 Rectangle{(float)screenX, (float)screenY, 50, 50},
...
bool WorldButton::isClickedDele() {
  ...
  return IsMouseButtonPressed(MOUSE_BUTTON_LEFT) &&
         CheckCollisionPointRec(
             GetMousePosition(),
             Rectangle{float(screenX), float(screenY), float(50), float(50)});
}
