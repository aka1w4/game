...
private:
  ...
  int x, y, w, h, w2, h2, fontsize, screenX,
      screenY; // posisi x y, lebar tinggi ukuran frame, fontsize ukuran text,
               // screenX screenY posisi yang akan di gambar
...
public:
  ...
  Button(int x, int y, int w, int h, int fontsize, const char *text,
         Texture2D &img, std::function<void()> action);
  // @brief Draw button
...
private:
  ...
  int x, y, h, screenX, screenY; // posisi x y, lebar tinggi ukuran frame, dan
                                 // screenX screenY posisi yang akan di gambar
  Texture2D *imgicon;            // texture image
  std::function<void()>
      actionDelete; // aksi yang akan di jalankan di ActionDelete()

public:
  /// @brief membuat world button baru
  WorldButton(int x, int y, int w, int h, int fontsize, Texture2D &img,
              Texture2D &imgicon, const WorldInfo &wi,
              std::function<void(const WorldInfo &)> action,
              std::function<void()> actionDelete)
      : Button(x, y, w, h, fontsize, wi.name.c_str(), img,
               [=]() { action(wi); }),
        path(wi.path), imgicon(&imgicon), x(x), y(y), h(h),
...
public:
  Textinput(int x, int y, int w, int h, int fontsize, Texture2D &img)
      : x(x), y(y), w(w), h(h), fontsize(fontsize), img(&img) {};
  void checkPos();
  void Draw();
  bool inTextInput();
  void EditInputText();
