...
const Vector2 &Player::GetPlayerpos() { return e.pos; }
...
void Player::DrawHeart() {
  for (int i = 0; i < maxHealth / 2; i++) {
    ...
    if (health > hpIndex + 1) {
      DrawTexturePro(*healthimg, Rectangle{0, 0, 11, 11},
                     Rectangle{(float)screenX, 0, 33, 33}, Vector2{0, 0}, 0.0f,
                     WHITE); // full health
    } else if (health == hpIndex + 1) {
      DrawTexturePro(*healthimg, Rectangle{11, 0, 11, 11},
                     Rectangle{(float)screenX, 0, 33, 33}, Vector2{0, 0}, 0.0f,
                     WHITE); // half health
    } else {
      DrawTexturePro(*healthimg, Rectangle{22, 0, 11, 11},
                     Rectangle{(float)screenX, 0, 33, 33}, Vector2{0, 0}, 0.0f,
                     WHITE); // empty health
    }
  }
